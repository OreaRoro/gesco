...
const PointageList: React.FC = () => {
  const [loading, setLoading] = useState(true);
  const [pointages, setPointages] = useState<PointagePersonnel[]>([]);
  ...
  const loadSelectData = async () => {
    try {
      const [statutsList, types, personnel] = await Promise.all([
  ...
  const handleDelete = async (pointageId: number) => {
    try {
      ...
      if (confirmSuppression) {
        await pointageService.delete(pointageId);
  ...
  const handlePointageRapide = async (
    ...
    try {
      await pointageService.pointageRapide(personnelId, type);
      toast.success(
  ...
  const calculerHeuresTravail = (pointage: PointagePersonnel) => {
  ...
  return (
    <>
      <PageMeta
        title="Pointage du Personnel | Système de Gestion Scolaire"
        description="Gérer les pointages du personnel"
      />

      <PageBreadcrumb
        pageTitle="Pointage du Personnel"
        breadcrumbs={[
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-gray-600">
              Gérer les présences et absences du personnel
            </p>
          </div>

          <div className="flex gap-3">
            <Link
              to="/personnel/pointage/rapide"
                      ...
                      <TableRow key={pointage.id} className="hover:bg-gray-50">
                        ...
                        <TableCell className="px-5 py-6 text-gray-500">
                          <div className="flex gap-2">
                            <Link
                              to={`/personnel/pointage/${pointage.id}/modifier`}
